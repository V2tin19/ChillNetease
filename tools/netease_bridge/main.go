...
//export NeteaseSearchSongs
// NeteaseSearchSongs 全站搜索单曲（go-musicfox SearchService，Type=1）
// keyword: 搜索关键词；limit: 返回数量（默认 30）
// 返回: 网易云 search/get 原生 JSON（result.songs[]）
func NeteaseSearchSongs(keywordC *C.char, limitC *C.char) *C.char {
	if !initialized {
		lastError = "Not initialized"
		return nil
	}
	...
	if code != 200 || len(resp) == 0 {
		lastError = "Search failed (code=" + strconv.Itoa(int(code)) + ")"
		return nil
	}
